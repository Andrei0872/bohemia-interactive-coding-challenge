...
const SORTING_CRITERION = new Map([
  ["Name", "Name"],
  ["Price", "Price"],
]);
...
function Products() {
  ...
  const shownProducts = useMemo(() => {
    ...
    result.sort((a, b) => {
      if (sortingCriterion === "Name") {
        return isSortingAsc ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name);
      }
  ...
  const sortingCriterionChangeHandler = (ev: any) => {
    setSortingCriterion(ev.target.value);
  }

  const sortingOrderChangeHandler = () => {
      ...
      <div className="flex justify-between items-center">
        ...
        <div className='flex gap-3'>
          <div className='flex text-gray-2 items-center gap-1'>
            ...
            <svg className="cursor-pointer" onClick={() => sortingOrderChangeHandler()} width="15" height="15" viewBox="0 0 15 15" fill="#9B9B9B" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className='flex items-center gap-1'>
            <select value={sortingCriterion} onChange={(ev) => sortingCriterionChangeHandler(ev)} name="sorting-criterion" id="sorting-criterion">
              {[...SORTING_CRITERION.keys()].map(sc => (
